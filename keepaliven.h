...
void MergeAndSelect(population *pop1_ptr,memory *pop2_ptr,population *pop3_ptr,int gen);
...
void keepalive(Population *pop1_ptr,Population *pop2_ptr,Population *pop3_ptr,int gen)
{
    ...
    for(i = 0; i < popsize; i++)
    {
        ...
        /* Initializing the dummy fitness to zero */
...
void MergeAndSelect(population *pop1_ptr,memory *pop2_ptr,population *pop3_ptr,int gen)
{
  ...
  // for population
    ...
    /*Initializing the flags of population to zero */
    for(i = 0; i < (popsize + MemorySize); i++)
    {
        globalpop.flag[i] = 0;
    }
    // decide which all solutions belong to the pop3
    ...
    k = 0;
    for(i = 0,k = 0; i < (popsize + MemorySize) && k < popsize; i++)
    {
        if(nchrom > 0)
        {
            if(globalpop.flag[i] == 1)
